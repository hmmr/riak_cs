...
-record(state, { riak_client :: pid()
               ...
               , response :: undefined | {ok, list_roles_response()} | {error, term()}
               }
       ).
...
-define(FOLD_OBJECTS_BATCH_SIZE, 100).


-spec start_link(riak_client(), list_roles_request()) ->
    {ok, pid()} | {error, term()}.
start_link(RcPid, Request) ->
    start_link(RcPid, Request, ?FOLD_OBJECTS_BATCH_SIZE).

-spec start_link(riak_client(), list_roles_request(), pos_integer()) ->
    {ok, pid()} | {error, term()}.
start_link(RcPid, Request, FoldObjectsBatchSize) ->
    BatchSize2 = max(2, FoldObjectsBatchSize),
    gen_fsm:start_link(?MODULE, [RcPid, Request, BatchSize2], []).
