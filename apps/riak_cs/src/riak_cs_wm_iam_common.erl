...
service_available(RD, Ctx=#rcs_iam_context{submodule=Mod, rc_pool=Pool}) ->
...
malformed_request(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                           exports_fun=ExportsFun,
                                           stats_prefix=StatsPrefix}) ->
...
valid_entity_length(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                             exports_fun=ExportsFun}) ->
...
-type validate_checksum_response() :: {error, term()} |
                                      {halt, pos_integer()} |
                                      boolean().
-spec validate_content_checksum(#wm_reqdata{}, #rcs_iam_context{}) ->
          {validate_checksum_response(), #wm_reqdata{}, #rcs_iam_context{}}.
validate_content_checksum(RD, Ctx=#rcs_iam_context{submodule=Mod, exports_fun=ExportsFun}) ->
...
allowed_methods(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                         exports_fun=ExportsFun}) ->
...
-spec content_types_accepted(#wm_reqdata{}, #rcs_iam_context{}) -> {[{string(), atom()}], #wm_reqdata{}, #rcs_iam_context{}}.
content_types_accepted(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                                exports_fun=ExportsFun}) ->
...
-spec content_types_provided(#wm_reqdata{}, #rcs_iam_context{}) -> {[{string(), atom()}], #wm_reqdata{}, #rcs_iam_context{}}.
content_types_provided(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                                exports_fun=ExportsFun}) ->
...
-spec generate_etag(#wm_reqdata{}, #rcs_iam_context{}) -> {string(), #wm_reqdata{}, #rcs_iam_context{}}.
generate_etag(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                       exports_fun=ExportsFun}) ->
...
-spec last_modified(#wm_reqdata{}, #rcs_iam_context{}) -> {calendar:datetime(), #wm_reqdata{}, #rcs_iam_context{}}.
last_modified(RD, Ctx=#rcs_iam_context{submodule=Mod,
                                       exports_fun=ExportsFun}) ->
...
-spec delete_resource(#wm_reqdata{}, #rcs_iam_context{}) -> {boolean() | {halt, non_neg_integer()}, #wm_reqdata{}, #rcs_iam_context{}}.
delete_resource(RD, Ctx=#rcs_iam_context{submodule=Mod,exports_fun=ExportsFun}) ->
...
-spec to_xml(#wm_reqdata{}, #rcs_iam_context{}) ->
                    {binary() | {'halt', non_neg_integer()}, #wm_reqdata{}, #rcs_iam_context{}}.
to_xml(RD, #rcs_iam_context{user = User,
                            role = _Role,
                            submodule = Mod,
...
-spec to_json(#wm_reqdata{}, #rcs_iam_context{}) ->
          {binary() | {'halt', non_neg_integer()}, #wm_reqdata{}, #rcs_iam_context{}}.
to_json(RD, #rcs_iam_context{user = User,
                             role = _Role,
                             submodule = Mod,
                             exports_fun = ExportsFun} = Ctx) ->
...
post_is_create(RD, #rcs_iam_context{submodule = Mod,
                                    exports_fun = ExportsFun} = Ctx) ->
    resource_call(Mod, post_is_create, [RD, Ctx], ExportsFun).

create_path(RD, #rcs_iam_context{submodule = Mod,
                                 exports_fun = ExportsFun} = Ctx) ->
    resource_call(Mod, create_path, [RD, Ctx], ExportsFun).

process_post(RD, #rcs_iam_context{submodule = Mod,
                                  exports_fun = ExportsFun} = Ctx) ->
    resource_call(Mod, process_post, [RD, Ctx], ExportsFun).

resp_body(RD, #rcs_iam_context{submodule = Mod,
                               exports_fun = ExportsFun} = Ctx) ->
    resource_call(Mod, resp_body, [RD, Ctx], ExportsFun).

multiple_choices(RD, #rcs_iam_context{submodule = Mod,
                                      exports_fun = ExportsFun} = Ctx) ->
    try
...
-spec accept_body(#wm_reqdata{}, #rcs_iam_context{}) ->
          {boolean() | {'halt', non_neg_integer()}, #wm_reqdata{}, #rcs_iam_context{}}.
accept_body(RD, #rcs_iam_context{submodule = Mod,
                                 exports_fun = ExportsFun,
                                 user = User} = Ctx) ->
...
produce_body(RD, #rcs_iam_context{user = User,
                                  role = _Role,
                                  submodule = Mod,
                                  exports_fun = ExportsFun} = Ctx) ->
...
-spec finish_request(#wm_reqdata{}, #rcs_iam_context{}) -> {boolean(), #wm_reqdata{}, #rcs_iam_context{}}.
finish_request(RD, #rcs_iam_context{riak_client = RcPid,
...
-spec authorize(#wm_reqdata{}, #rcs_iam_context{}) -> {boolean() | {halt, non_neg_integer()}, #wm_reqdata{}, #rcs_iam_context{}}.
authorize(RD, #rcs_iam_context{submodule = Mod,
                               exports_fun = ExportsFun} = Ctx) ->
...
authenticate(User, UserObj, RD, #rcs_iam_context{auth_module = AuthMod,
                                                 submodule = Mod} = Ctx, AuthData)
...
post_authentication(AuthResult, RD, Ctx = #rcs_iam_context{submodule=Mod}, AnonOk) ->
    case post_authentication(AuthResult, RD, Ctx, fun authorize/2, AnonOk) of
        {false, _RD2, Ctx2} = FalseRet ->
            riak_cs_dtrace:dt_wm_return({?MODULE, Mod},
                                        <<"forbidden">>, [],
                                        [riak_cs_wm_utils:extract_name(Ctx2#rcs_iam_context.user),
                                         <<"false">>]),
            FalseRet;
        {Rsn, _RD2, Ctx2} = Ret ->
            Reason =
...
post_authentication({ok, User, _UserObj}, RD, Ctx, Authorize, _) ->
...
post_authentication({error, reqtime_tooskewed} = Error, RD,
                    #rcs_iam_context{response_module = ResponseMod} = Ctx, _, _) ->
...
update_stats(_RD, #rcs_iam_context{stats_key=no_stats}) ->
    ok;
update_stats(_RD, #rcs_iam_context{stats_prefix=no_stats}) ->
    ok;
update_stats(RD, #rcs_iam_context{start_time=StartTime,
...
-spec default_content_types_provided(#wm_reqdata{}, #rcs_iam_context{}) ->
          {[{string(), atom()}],
           #wm_reqdata{},
           #rcs_iam_context{}}.
default_content_types_provided(RD, Ctx=#rcs_iam_context{api=oos}) ->
...
default_produce_body(RD, Ctx=#rcs_iam_context{submodule=Mod,
